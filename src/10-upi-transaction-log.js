...
export function analyzeUPITransactions(transactions) {
  ...
  const totalCredit = validTransactions.filter(txn => txn.type === "credit")
                                        .reduce((sum, txn)=> sum + txn.amount, 0);

  const totalDebit = validTransactions.filter(txn => txn.type === "debit")
                                      .reduce((sum, txn) => sum + txn.amount, 0);
  ...
  return{
    ...
    avgTransaction, highestTransaction,
    categoryBreakdown, frequentContact,
    allAbove100, hasLargeTransaction
  };
}
